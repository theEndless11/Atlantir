...
export function useVoiceMode(workspaceId: string) {
  ...
  async function startListening() {
    ...
    socket = new WebSocket(
      `wss://api.deepgram.com/v1/listen?model=nova-2&language=en&smart_format=true&interim_results=true&endpointing=800&vad_events=true`,
      ['token', key]
    )
    ...
    socket.onerror  = () => { error.value = 'Voice connection failed'; stopMic() }
    socket.onclose  = (e) => {
      isListening.value = false
      if (e.code !== 1000 && e.code !== 1001) error.value = `Connection closed (${e.code})`
    }
  }
